// components/home/ArticleFeedSection.tsx — Server Component
// Fetch artikel secara mandiri agar Suspense boundary ini tidak memblokir
// bagian lain halaman (kategori filter, shell).
...
function buildFeedItems(articles: Article[], every: number = 3) {
  const items: Array<
    { kind: "article"; data: Article } | { kind: "ad" }
  > = [];
...
export default async function ArticleFeedSection({ search, category }: Props) {
  // Fetch artikel dan kategori aktif secara paralel
  ...
  // Nama kategori aktif untuk heading section
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── BILLBOARD AD (di bawah hero, sebelum grid) ── */}
      {!search && <AdBillboard />}
